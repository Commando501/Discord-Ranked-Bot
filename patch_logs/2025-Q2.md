...
- Created a new rank definition system with customizable tiers
- Integrated rank tiers with the reward system in the web interface
- Updated leaderboards to use the defined rank tiers
- Modified seeder to apply default ranks if none are defined
- **Files Modified**:
  - Added `shared/rankSystem.ts`
  - Updated `shared/botConfig.ts`
  - Updated `client/src/components/config/season-config.tsx`
  - Updated `client/src/pages/leaderboards.tsx`

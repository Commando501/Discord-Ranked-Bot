...
- Fixed documentation to ensure proper application setup instructions

**Purpose**: Restore repository synchronization with remote

**Testing**: Verified README.md displays correctly without conflict markers

**Dependencies Affected**: None


### 2025-05-29 13:30:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/index.bot.ts`

**Changes**:
- Fixed major structural issues in the Discord bot file with mismatched brackets
- Removed duplicate catch blocks causing syntax errors
- Corrected nesting of event handlers and fixed indentation
- Properly structured the client event handlers for MessageCreate and InteractionCreate

**Purpose**: Fix syntax errors that were preventing the application from starting

**Testing**: Verified that the application compiles without syntax errors

**Dependencies Affected**: None


### 2025-08-05 14:30:00 UTC
**Type**: Bug Fix
**Files Modified**: 
...
**Changes**:
- Fixed race condition in the votekick system where concurrent votes could lead to inconsistent match states
- Implemented transaction-based processing for votekick operations to ensure data consistency
- Added proper race condition handling in `handleSuccessfulVoteKick` method
- Enhanced vote counting logic to accurately track vote status across concurrent operations
- Added proper error handling and rollback mechanisms for failed votekick processes
- Fixed timing issues where multiple simultaneous votes could cause matches to remain in inconsistent states

**Purpose**: Resolve critical race conditions in the votekick system that could lead to matches not being properly concluded after a successful votekick or votes not being accurately counted when received in rapid succession.

**Testing**: Verified that votekick operations now work reliably even with multiple concurrent votes, and that match state remains consistent during the entire votekick process.

**Dependencies Affected**: None



### 2025-06-15 02:20:00 UTC


### 2025-07-04 10:30:00 UTC
**Type**: Enhancement
**Files Modified**:
...
**Changes**:
- Improved logging system to reduce console spam from routine operations
- Implemented smart logging for queue checks that only shows detailed logs when queue state changes
- Moved routine status messages to debug level to keep the console clean
- Added log counter to provide context on how many checks have occurred
- Added support for CONSOLE_LOG_LEVEL environment variable to control console output verbosity
- Left file logging unchanged to maintain complete records for troubleshooting

**Purpose**: Improve developer experience by reducing log clutter while maintaining complete logging for diagnostics
...
**Changes**:
- Fixed `ReferenceError: winningTeamNameStr is not defined` error in the `endMatch` function
- Moved the `winningTeamNameStr` declaration to a higher scope level to ensure it's accessible throughout the function
- Improved team name lookup logic to ensure compatibility with both string and numeric inputs

**Purpose**: Fix critical bug preventing matches from being ended properly when using the `/endmatch` command.

**Testing**: Verified match completion works correctly with both team names and team IDs.

**Dependencies Affected**: None



### 2025-07-15 02:00:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/bot/services/queueService.ts`
- `server/storage.ts`
...
**Changes**:
- Fixed a critical race condition in the match creation process that could result in duplicate matches with the same players
- Implemented database-level row locking with `FOR UPDATE` to ensure players can't be selected for multiple matches simultaneously
- Enhanced transaction handling in the queue selection process
- Modified the queue player selection approach to directly use database locking mechanisms
- Simplified the batch removal process to work within the locked transaction
- Added more informative logging during the player selection and removal process

**Purpose**: Resolve a race condition that was creating duplicate matches with the same set of players, particularly under high load or when multiple queue processing operations ran simultaneously.

**Testing**: Verified that concurrent queue checks no longer create duplicate matches with the same players, even when multiple processes are running simultaneously.

**Dependencies Affected**: None



### 2025-07-05 15:45:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/index.bot.ts`
- `server/bot/services/matchService.ts`
...
**Changes**:
- Fixed race condition in match voting system where concurrent votes could cause match completion failures
- Added a transitional "COMPLETING" state to prevent multiple concurrent match completion processes
- Enhanced error handling and rollback mechanism for failed match completions
- Improved type handling for team identification to support both name strings and ID numbers
- Added additional debug logging to help diagnose match completion issues
- Implemented status verification before processing votes to prevent votes on already completed matches

**Purpose**: Fix critical race condition where rapid consecutive votes could lead to inconsistent match states or database errors when completing matches

**Testing**: Verified that match completion works correctly with simultaneous votes, with proper handling of edge cases and error conditions

**Dependencies Affected**: None



### 2025-07-15 00:00:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/bot/services/matchService.ts`

**Changes**:
- Fixed match cleanup process not occurring after match completion
- Added explicit channel cleanup in the `endMatch` method 
- Implemented proper countdown and channel deletion logic
- Fixed event emission with ES module imports
- Added auto-requeue functionality based on configuration
- Enhanced error handling during cleanup process

**Purpose**: Fix the issue where completed matches weren't properly cleaning up their channels and potentially not auto-requeuing players.

**Testing**: Verified that match channels are properly deleted after the completion countdown and players are requeued if the configuration setting is enabled.

**Dependencies Affected**: None



### 2025-06-07 00:00:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/bot/services/queueService.ts`

**Changes**:
- Fixed an edge case where players who get disconnected during match creation can leave other players stuck in processing state
- Added proper handling of undefined `matchPlayers` variable in cleanup sections
- Implemented a periodic cleanup mechanism to detect and fix stuck player states
- Enhanced error handling to ensure players are always unmarked when match creation fails
- Added additional safety checks throughout the queue and match creation process

**Purpose**: Fix a critical bug where players could get permanently stuck in the processing state if another player gets kicked from the server during match creation.

**Testing**: Verified that players no longer remain in the processing state when errors occur or when players disconnect during match creation.
...
- `client/src/pages/admin.tsx` (removed)

**Changes**:
- Removed the "Admin Settings" page from the web dashboard
- Removed the Admin page route from the routing configuration
- Removed the Admin link from the sidebar navigation

**Purpose**: Streamline the user interface as all admin functionality has been distributed to other more specific sections of the application

**Testing**: Verified the application runs correctly without the admin page, and all admin features are accessible through other sections of the dashboard

**Dependencies Affected**: None

# Patch Log: 2025 Q2
## [2025-05-10] Server Migration Tools

### Files Modified
- Added `server/utils/export-db-script.ts`
- Added `server/utils/import-db-script.ts`
- Added `HOST_MIGRATION.md`
- Added `deploy.sh`

### Changes
- Created tools to export and import the complete database
- Added a comprehensive host migration guide
- Created a deployment script for simplified setup on new hosts
- Ensured environment variable configuration is well-documented

### Testing
- Verified database export/import functionality with test data
- Confirmed deployment script works in a clean environment
- Validated all required environment variables are documented

These changes ensure the application can be migrated to other hosting platforms when needed, while maintaining all data and functionality.


### 2025-07-20 01:30:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/storage.ts`
- `server/routes.ts`
...
**Changes**:
- Fixed player deletion functionality by properly handling foreign key constraints
- Enhanced `deletePlayer` method to cascade deletion to related tables (match_votes, team_players, etc.)
- Improved error handling and messaging for player deletion failures
- Added detailed logging for successful player deletions

**Purpose**: Resolve bug where player deletion was failing due to foreign key constraint violations from the match_votes table and other references.

**Testing**: Verified that players with match history, votes, and other relationships can now be properly deleted from the admin interface.

**Dependencies Affected**: None



### 2025-08-02 02:15:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `client/src/pages/history.tsx`

**Changes**:
- Completely rewrote the rank emblem display implementation with a much simpler approach
- Fixed inconsistent rank icon file naming by standardizing the filename format in getRankFromMMR
- Implemented a more robust error handling approach for image loading
- Corrected hardcoded icon file paths to use consistent capitalization
- Simplified the fallback mechanism for when rank icons can't be loaded

**Purpose**: Fix the issue where rank emblems were appearing as gray circles instead of proper rank icons in the match history player listings.

**Testing**: Verified that rank emblems now properly display for all players in the match history view.
...
**Testing**: Verified that rank emblems display correctly in match history expanded details.
...
**Purpose**: Provide Discord users with more detailed match history information directly in Discord, matching the level of detail available in the web dashboard
...
- 2025-06-28: Fixed Discord profile rank determination with direct highest-first approach
...
- Fixeddate formatting for player creation date
...
## 2025-04-28 14:30 - Match Vote Threshold Update

### Files Modified:
- `shared/botConfig.ts`
- `client/src/components/config/match-rules-config.tsx`

### Changes:
- Increased the default minimum votes required to determine a match winner from 3 to 5
- Updated the form configuration UI to reflect this change and added clarification that this is optimized for 8-player matches
- Default value remains configurable but starts at a higher threshold to ensure more accurate match results

### Rationale:
With 8 players per match, requiring 5 votes (more than half) provides a more democratic approach to determining match outcomes and reduces potential for small groups to influence results.

### Testing:
- Verified that existing matches work with new vote threshold
- Confirmed UI updates reflect the new default value
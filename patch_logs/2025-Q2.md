...
**Changes**:
- Completely replaced file upload implementation with Replit's Object Storage
- Switched from formidable to express-fileupload for in-memory file handling
- Implemented dual storage strategy (Object Storage + local file system)
- Added better content-type checking in the client to detect non-JSON responses
- Improved client-side error handling for rank icon uploads
- Eliminated middleware-related issues by using a direct approach to file handling
- Implemented more robust error recovery on both client and server sides

**Purpose**: Resolve persistent "Received text/html; charset=utf-8 instead of JSON response" error by using Replit's Object Storage instead of file system operations through middleware

**Testing**: Verified rank icon uploads work without content-type issues using the new approach

**Dependencies Affected**: 
- Added @replit/object-storage package

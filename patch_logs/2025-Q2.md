...
### 2025-07-31 12:00:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/routes.ts`

**Changes**:
- Fixed the match history API endpoint to include MMR change data for players
- Enhanced the `/api/matches/history` endpoint to retrieve and include player MMR changes
- Added player match history lookup to determine actual MMR changes for each player
- Ensured completed matches show proper MMR gain/loss values instead of "0"

**Purpose**: Fix the issue where MMR changes were not being displayed in the match history UI despite being stored in the database.

**Testing**: Verified that match history now correctly displays MMR changes for each player instead of showing "0" values.

...
### 2025-06-28 06:15:00 UTC
**Type**: Bug Fix
**Files Modified**: 
- `server/discord/commands/list.ts`

**Changes**:
- Fixed issue where Discord list command was failing to load detailed rank tiers from the configuration file
- Changed CommonJS `require()` statements to ES module imports using `import * as fs from 'fs/promises'`
- Implemented proper async file handling with try/catch blocks
- Ensured error messages are more descriptive to help with debugging

**Purpose**: Fix "Error loading detailed rank tiers from config: ReferenceError: require is not defined" error that was preventing the Discord /list command from displaying detailed rank information.

**Testing**: Verified that the list command can now properly load the rank tier configuration without throwing errors.

...
### 2025-07-04 11:45:00 UTC
**Type**: Enhancement
**Files Modified**: 
- `server/discord/commands/list.ts`

**Changes**:
- Enhanced `/list` command to display player rank tier names alongside MMR values
- Added integration with the rank system from `shared/rankSystem.ts`
- Updated all queue embed creation and refresh logic to include rank information
- Added support for displaying proper rank tier names based on player MMR
- Improved player list formatting to clearly show rank tiers

**Purpose**: Enhance visibility of player ranks in the queue list, making it easier to see the skill distribution at a glance

**Testing**: Verified that the correct rank tier names appear next to player MMR in both initial and refreshed queue embeds

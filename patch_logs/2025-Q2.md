...
**Changes**:
- Added new API endpoint `/api/admin/export-database` to extract data from all database tables
- Implemented client-side functionality to download database as JSON file
- Added loading state to the export button to improve user experience
- Implemented error handling for database export process
- Export includes data from all tables: players, queue, matches, teams, teamPlayers, matchVotes, voteKicks, and voteKickVotes

**Purpose**: Provide administrators with the ability to export all database data for backup, migration, or analysis purposes

**Testing**: Verified that all table data is correctly exported to a downloadable JSON file with proper formatting

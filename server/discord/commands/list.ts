...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    if (queuePlayers.length > 0) {
      ...
      try {
        // Use import for ES modules instead of require
        ...
        if (fs.existsSync(configPath)) {
          ...
          if (configData.seasonManagement && configData.seasonManagement.rankTiers && 
            ...
            logger.info(`Loaded ${rankTiers.length} detailed rank tiers from config file for list command`);
          }
        }
      } catch (configError) {
            ...
            try {
              ...
              if (fs.existsSync(configPath)) {
                ...
                if (configData.seasonManagement && configData.seasonManagement.rankTiers && 
                  ...
                  // Replace with config ranks if they exist - this ensures we use the complete set
                  rankTiers = configData.seasonManagement.rankTiers;
                  logger.info(`Loaded ${rankTiers.length} detailed rank tiers from config file for list command update`);
                }
              }
            } catch (configError) {
              logger.error(`Error loading detailed rank tiers from config: ${configError}`);

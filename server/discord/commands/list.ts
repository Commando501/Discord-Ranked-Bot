    ...
    if (queuePlayers.length > 0) {
      ...
      const queueListPromises = queuePlayers.map(async (entry, index) => {
        ...
        try {
          ...
          if (fs.existsSync(configPath)) {
            ...
            if (configData.seasonManagement && configData.seasonManagement.rankTiers && 
              ...
              logger.info(`Using ${rankTiers.length} detailed rank tiers directly from config file for list command`);
        ...
        if (foundTier) {
          ...
          logger.info(`CORRECTED ALGORITHM: Selected rank "${playerRank.name}" for player with MMR ${entry.player.mmr}.`);
            ...
            try {
              ...
              if (fs.existsSync(configPath)) {
                ...
                if (configData.seasonManagement && configData.seasonManagement.rankTiers && 
                  ...
                  // Replace with config ranks if they exist - this ensures we use the complete set
                  rankTiers = configData.seasonManagement.rankTiers;
                  logger.info(`Using ${rankTiers.length} detailed rank tiers directly from config file for list update`);
                  // Log all tier names for debugging
